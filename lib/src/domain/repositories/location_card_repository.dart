...
class LocationCardRepository {
  ...
  Future<List<LocationCardModel>> fetchEventLocations(int eventId, String userUid) async {
    try {
      // 1. 모든 장소 카드 가져오기
      final List<LocationCardModel> locations = await _locationService.fetchEventLocations(eventId);
      
      // 2. 방문 여부 가져오기
      final Set<int> visitedSeqs = await _locationService.fetchVisitedLocationSeqs(userUid);
